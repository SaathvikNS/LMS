...
const EditProfileMain = ({ navigation }) => {
  ...
  const submitpressed = async () => {
    ...
    if (!newContact) {
      newContact = profilecontact;
    }
    if (!newDOJ) {
      newDOJ = profiledateofjoin;
    }
    if (!newDOB) {
      newDOB = profiledateofbirth;
    }
    await setemail(newMail);
    await setprofilecontact(newContact);
    await setprofiledateofjoin(newDOJ);
    await setprofiledateofbirth(newDOB);
    ...
    try {
      const { data } = await axios.post(
        "http://192.168.50.79:8000/api/user/updateprofile",
        {
          ...
          empid,
          email: newMail || email,
          profilecontact: newContact || profilecontact,
          profiledateofjoin: newDOJ || profiledateofjoin,
          profiledateofbirth: newDOB || profiledateofbirth,
        }
      );
      console.log(data);
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.contentwrapper}>
          ...
          <EditProfileInfoBlocks
            ...
            editable={true}
            placeholder={profilecontact || "Your Contact?"}
          />
          <EditProfileInfoBlocks
            ...
            editable={true}
            placeholder={profiledateofjoin || "dd/mm/yyyy"}
          />
          <EditProfileInfoBlocks
            ...
            editable={true}
            placeholder={profiledateofbirth || "dd/mm/yyyy"}
          />
          <EditProfileInfoBlocks
            source={require("../../../assets/profileicons/gender.png")}
            keyplace={"Gender"}
            valueplace={profilegender}
            editable={false}
          />
        </View>
